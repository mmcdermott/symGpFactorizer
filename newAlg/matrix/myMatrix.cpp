...
Matrix::Matrix(const Matrix& copy)
  :numRows_(copy.rows()), numCols_(copy.cols())
{
...
Matrix::Matrix(const size_t rows, const size_t cols) 
  :numRows_(rows), numCols_(cols), data_(new scalar[rows*cols])
{
...
Matrix::Matrix(const size_t dim)
  :numRows_(dim), numCols_(dim), data_(new scalar[dim*dim])
{
  size_t size = dim*dim;
  for (size_t i = 0; i < size; i++)
    this->data_[i] = 0;
...
//Matrix::Matrix(const IdentityMatrix& copy) 
//  :numRows_(copy.rows()), numCols_(copy.cols()), data_(new scalar[rows*cols])
//{
//  size_t rows = copy.rows();
//  size_t size = rows**2;
//  for (size_t i = 0; i < size; i++)
//    this->data_[i] = 0;
//  for (size_t index = 0; i < copy.rows(); i++)
//    (*this)(index,index) = 1;
//}
...
//  Orthogonality: 
bool orthogonal(bool debug = false) const {
  for (size_t i = 0; i < this->numCols(); ++i) {
    for (size_t j = i+1; j < this->numCols(); ++j) {
...
Matrix  Matrix::operator* (const Matrix& rhs) const {
  if (this->numCols_ != rhs.numRows_) {
    std::cout << "You can't multiply matricies of this dimension.";
    throw;
  }
